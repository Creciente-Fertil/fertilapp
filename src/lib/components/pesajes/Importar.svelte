<script>
    import { importarAnimales, importarPesajes } from "$lib/java/establecimientos/establecimientosback";
    import estilos from "$lib/stores/estilos";
    import Swal from "sweetalert2";
    import * as XLSX from "xlsx";

    let { cabid = "",getPesajes=async()=>{} } = $props();
    let usuarioid = $state("");
    ...
    async function procesarArchivo() {
        ...
        loading = true;
        try {
            const resultado = await importarPesajes(cabid, selectedFile);
            await getPesajes()
            filename = ""
            
            Swal.fire(
                "Éxito importar pesajes",
                "Se logró importar con éxito",
                "success",
            );
        } catch (error) {
            Swal.fire(
                "Error importar pesajes",
