<script>
    import { importarAnimales, importarObservaciones } from "$lib/java/establecimientos/establecimientosback";
    import estilos from "$lib/stores/estilos";
    import Swal from "sweetalert2";
    import * as XLSX from "xlsx";

    let { cabid = "",getObservaciones=async()=>{} } = $props();
    let usuarioid = $state("");
    ...
    async function procesarArchivo() {
        ...
        loading = true;
        try {
            const resultado = await importarObservaciones(cabid, selectedFile);
            await getObservaciones()
            filename = ""
            
            Swal.fire(
                "Éxito importar observaciones",
                "Se logró importar con éxito",
                "success",
            );
        } catch (error) {
            Swal.fire(
                "Error importar observaciones",
