<script>
    ...
    import { getMovementType } from "$lib/java/movimientos/movimientosback";
    let esdev = import.meta.env.VITE_DEV == "si";

    let innerWidth = $state(0);
    let innerHeight = $state(0);
    let esCelu = $derived(innerWidth <= 1100);
    let tipos = getMovementType().filter(t=>t.cod<1)
    let {
        movimientosrows = [],
        cabnombre = "",
        selecthash = {},
                ...
                >
                    <div class="text-md text-gray-700 dark:text-white">
                        <span class="hidden xl:block"
                            >Total de Movimientos seleccionados: {Object.keys(
                                selecthash,
                            ).length}</span
                        >
                        <span class="xl:hidden">
                            Movimientos seleccionados: {Object.keys(selecthash)
                                .length}
                        </span>
                    </div>
                    <div class="flex flex-wrap gap-2">
