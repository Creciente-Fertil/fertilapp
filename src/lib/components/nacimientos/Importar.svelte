<script>
    import { importarAnimales, importarLotes, importarNacimientos } from "$lib/java/establecimientos/establecimientosback";
    import estilos from "$lib/stores/estilos";
    import Swal from "sweetalert2";
    import * as XLSX from "xlsx";

    let { cabid = "",getNacimientos=async()=>{} } = $props();
    let usuarioid = $state("");
    ...
    async function procesarArchivo() {
        ...
        loading = true;
        try {
            const resultado = await importarNacimientos(cabid, selectedFile);
            await getNacimientos()
            filename = ""
            Swal.fire(
                "Éxito importar nacimientos",
                "Se logró importar con éxito",
                "success",
            );
        } catch (error) {
            Swal.fire(
                "Error importar nacimientos",
