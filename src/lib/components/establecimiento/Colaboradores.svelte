<script>
    ...
    async function asociar() {
        ...
        if (!verificar) {
            Swal.fire(
                "Error guardar",
                `No tienes el nivel de la cuenta para tener más colaboradores`,
                "error",
            );
            return;
        }
        ...
        if (userid == origenusuarioid) {
            Swal.fire(
                "Error colaborador",
                "No puedes asociarte al establecimiento",
                "error",
            );
            return;
        }
        ...
        if (existecolab) {
            ...
            try {
                ...
                const recordestxcolab = await pb
                    .collection("estxcolabs")
                    .create(dataestxcolab);
                //Debo crear los permisos para el colaborador
                let datapermisos = {
                    estxcolab: recordestxcolab.id,
                    permisos: "",
                };
                const recordpermisos = await pb
                    .collection("permisos")
                    .create(datapermisos);
                Swal.fire(
                    "Éxito asociar",
                    "Se pudo asociar el usuario",
                    "success",
                );
    ...
    <div class={``}>
        ...
        >
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <Secondary
                    onclick={() =>
                        !mostrarasociacion ? openAsociar() : closeAsociar()}
                    conhijo={true}
                >
                ...
                {#if asociado}
                    <div>
                        <Danger onclick={confirmDesasociar}>
                            <X size="size-4" margenes="" />
                            Desasociar
                        </Danger>
                    </div>
                {/if}
            ...
            {#if malcorreo}
                <div class="label">
                    <span class="label-text-alt text-red-500"
                        >Debe escribir el correo de un usuario de esta
                        aplicación</span
                    >
                </div>
            {/if}
        ...
        <div class="form-control">
            ...
            <label class="input-group">
                ...
                {#if malnombre}
                    <div class="label">
                        <span class="label-text-alt text-red-500"
                            >Debe escribir el nombre del colaborador</span
                        >
                    </div>
                {/if}
            </label>
            <label for="ape2" class="label">
            ...
            <label class="input-group">
                ...
                {#if malapellido}
                    <div class="label">
                        <span class="label-text-alt text-red-500"
                            >Debe escribir el apellido del colaborador</span
                        >
                    </div>
                {/if}
            </label>
            ...
            <label class="input-group">
                ...
                {#if malemail}
                    <div class="label">
                        <span class="label-text-alt text-red-500"
                            >Debe escribir el email del colaborador</span
                        >
                    </div>
                {/if}
            </label>
            ...
            <label class="input-group">
                ...
                {#if malcontra}
                    <div class="label">
                        <span class="label-text-alt text-red-500"
                            >Debe escribir la contraseña del colaborador</span
                        >
                    </div>
                {/if}
            </label>
