<script>
    import { importarAnimales } from "$lib/java/establecimientos/establecimientosback";
    import estilos from "$lib/stores/estilos";
    import Swal from "sweetalert2";
    import * as XLSX from "xlsx";

    let { cabid = "",getAnimales=async()=>{} } = $props();
    let usuarioid = $state("");
    ...
    async function procesarArchivo() {
        ...
        loading = true;
        try {
            const resultado = await importarAnimales(cabid, selectedFile);
            await getAnimales()
            filename = ""
            
            Swal.fire(
                "Éxito importar animales",
                "Se logró importar con éxito",
                "success",
            );
        } catch (error) {
            Swal.fire(
                "Error importar animales",
