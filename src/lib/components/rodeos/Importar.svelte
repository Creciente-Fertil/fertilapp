<script>
    import { importarAnimales, importarLotes, importarRodeos } from "$lib/java/establecimientos/establecimientosback";
    import estilos from "$lib/stores/estilos";
    import Swal from "sweetalert2";
    import * as XLSX from "xlsx";

    let { cabid = "",getRodeos=async()=>{} } = $props();
    let usuarioid = $state("");
    ...
    async function procesarArchivo() {
        ...
        loading = true;
        try {
            const resultado = await importarRodeos(cabid, selectedFile);
            await getRodeos()
            filename = ""
            Swal.fire(
                "Éxito importar rodeos",
                "Se logró importar con éxito",
                "success",
            );
        } catch (error) {
            Swal.fire(
                "Error importar rodeos",
